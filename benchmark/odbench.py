...
class ODBench(BaseBenchmark):

    nr_hyperparameters = 500
    # Declaring the search space for ODBench
    param_space = OrderedDict([
        ('batch_size', [16, 512, int, True]),
        ('learning_rate', [0.0001, 0.1, float, True]),
        ('momentum', [0.1, 0.99, float, False]),
        ('weight_decay', [0.00001, 0.1, float, True])
    ])
    log_indicator = [True, True, False, True]
    max_budget = 20
    ...
    def __init__(self, path_to_json_file: str, dataset_name: str):
        ...
        hp_names = list(ODBench.param_space.keys())
        hp_configs = []
        for i in range(ODBench.nr_hyperparameters):
            hp_config = []
            config = self.api.query(
                dataset_name=self.dataset_name,
                tag='config',
                config_id=i,
            )
            for index, hp_name in enumerate(hp_names):
                if ODBench.log_indicator[index]:
    ...
    def get_data(self) -> None:
        # init data
        init_value = 0.
        for hp_index in range(0, ODBench.nr_hyperparameters):
            val_curve = self.api.query(
                dataset_name=self.dataset_name,
                config_id=hp_index,
                tag='Train/val_accuracy',
            )
            init_value += val_curve[0] / ODBench.nr_hyperparameters
        
        # data
        data = []
        for hp_index in range(ODBench.nr_hyperparameters):
            val_curve = self.api.query(
                dataset_name=self.dataset_name,
                config_id=hp_index,
                tag='Train/val_accuracy',
