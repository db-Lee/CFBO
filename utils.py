...
def get_utility_function(budget_limit, alpha, a=1, c=0):
    def U(budget, performance):
        u = performance - alpha*(budget / budget_limit)**a + c
        if type(u) == float:
...
class Logger:
    def __init__(
        ...
        wandb.init(entity=wandb_entity, project=wandb_project_name, name=exp_name, reinit=True)
